...
int main()
{
    ...
    try
    {
    std::thread th_1(MyThread::getOneCloset, std::ref(portfile),
                     std::ref(closets), lN, bN, books);
    std::this_thread::sleep_for(std::chrono::milliseconds(40));
    std::thread th_2(MyThread::getOneCloset, std::ref(portfile),
                     std::ref(closets), lN, bN, books);

    std::this_thread::sleep_for(std::chrono::milliseconds(40));
    std::thread th_3(MyThread::getOneCloset, std::ref(portfile),
                     std::ref(closets), lN, bN, books);

    std::this_thread::sleep_for(std::chrono::milliseconds(40));
    std::thread th_4(MyThread::getOneCloset, std::ref(portfile),
                     std::ref(closets), lN, bN, books);
